...
func loadPublicKey(path string) (sign.PublicKey, error) {
...
// SERVER OPERATIONS
func runServer(tlsConfig *tls.Config, serverPriv sign.PrivateKey, clientPub sign.PublicKey) {
	if *protocol == "tcp" {
		...
		for {
			...
			go handleTCPConnection(conn.(*tls.Conn), serverPriv, clientPub)
		}
	} else {
		...
		for {
			...
			go handleQUICSession(sess, serverPriv, clientPub)
		}
	}
}

func handleTCPConnection(conn *tls.Conn, serverPriv sign.PrivateKey, clientPub sign.PublicKey) {
	defer conn.Close()
	...
	if err := performMLDSAHandshake(conn, serverPriv, nil, true); err != nil {
	...
	handleConnWithVerify(conn, metrics, clientPub)
	log.Println("🛑 TCP connection handler finished")
}

func handleQUICSession(sess quic.Connection, serverPriv sign.PrivateKey, clientPub sign.PublicKey) {
	for {
		...
		go handleQuicStreamWithVerify(stream, sess, serverPriv, clientPub)
	}
}

func handleQuicStreamWithVerify(stream quic.Stream, sess quic.Connection, serverPriv sign.PrivateKey, clientPub sign.PublicKey) {
	defer stream.Close()
	
	if err := performMLDSAHandshake(stream, serverPriv, nil, true); err != nil {
		log.Println("ML-DSA handshake failed:", err)
		return
	}
	...
	handleConnWithVerify(stream, metrics, clientPub)
	log.Println("🛑 QUIC connection handler finished")
}

func handleConnWithVerify(rw io.ReadWriter, metrics *MetricsLogger, clientPub sign.PublicKey) {
	...
	// Verify signature
	hstart := time.Now()
	if ok := clientPub.Scheme().Verify(clientPub, data, signature, nil); !ok {
...
// CLIENT OPERATIONS
func runClient(tlsConfig *tls.Config, clientPriv sign.PrivateKey, serverPub sign.PublicKey) {
	data := make([]byte, *size)
	for i := range data {
		data[i] = byte(i % 256)
	}
	start := time.Now()
	sig, err := clientPriv.Sign(rand.Reader, data, crypto.Hash(0))
	if err != nil {
	...
	if *protocol == "tcp" {
		...
		if err := performMLDSAHandshake(conn, nil, serverPub, false); err != nil {
		...
		if err := performMLDSAHandshake(stream, nil, serverPub, false); err != nil {
			log.Fatal("ML-DSA handshake failed:", err)
		}		
		handshakeEnd = time.Now()
...
func main() {
	...
	if *mode == "server" {
		// Server uses its own private key and client's public key
		serverPriv, _, err := initMLDSAKeys("server_mldsa_priv.key", "server_mldsa_pub.key")
		if err != nil {
			log.Fatalf("Server ML-DSA key init failed: %v", err)
		}
		clientPub, err := loadPublicKey("client_mldsa_pub.key")
		if err != nil {
			log.Fatalf("Failed to load client public key: %v", err)
		}
		runServer(tlsConfig, serverPriv, clientPub)
	} else {
		// Client uses its own private key and server's public key
		clientPriv, _, err := initMLDSAKeys("client_mldsa_priv.key", "client_mldsa_pub.key")
		if err != nil {
			log.Fatalf("Client ML-DSA key init failed: %v", err)
		}
		serverPub, err := loadPublicKey("server_mldsa_pub.key")
		if err != nil {
			log.Fatalf("Failed to load server public key: %v", err)
		}
		runClient(tlsConfig, clientPriv, serverPub)
	}
}
