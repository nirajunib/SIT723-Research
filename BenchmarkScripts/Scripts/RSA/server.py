...
def start_tcp_server(verbose=False):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        conn, addr = s.accept()
        log(f"✅ TCP connection accepted from {addr}", verbose)

        with conn:
            received = b""
            start_time = time.time()  # Start measuring after accept()
            ...
            try:
                ...
                log(f"✅ Data verified. {len(data)} bytes in {connection_time:.2f}s", verbose)
